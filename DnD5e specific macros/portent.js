...
/////////////////////Function to: Determine the buttons/////////////////////////////
async function generateButtons(macroActor, item, itemName){
    ...
    let diceNumber = macroActor.items.getName("Wizard").data.data.levels < 14 ? 2 : 3; //sets up for Greater Portent where the player gets 3 dice at level 14.
    let myButtons = {};
    if (portentRolled !== undefined){
        for (let roll of portentRolled){
...
(async () => {
    ...
    const wizardActor = game.user.character || canvas.tokens.controlled[0].actor; //second option is for the GM.
    const portentItem = wizardActor.items.find(i => i.data.name.includes(itemName)); // finds that item name.

    if(!portentItem) return ui.notifications.warn("Your actor does not have the Portent feature");
    let myButtons = await generateButtons(wizardActor, portentItem, itemName); // creates the buttons, see function above.
    new Dialog({
