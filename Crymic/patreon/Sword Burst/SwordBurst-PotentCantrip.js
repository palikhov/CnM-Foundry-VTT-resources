...
(async () => {
    ...
    let classType = ["Sorcerer", "Warlock", "Wizard", "Artificer"];
    ...
    for (let target of get_target) {
        ...
        if (save.total >= dc) {
            damageResult.push(`<div class="midi-qol-flex-container"><div class="midi-qol-target-npc midi-qol-target-name" id="${target.id}"> ${target.name} saves with a ${save.total}</div><div><img src="${target.data.img}" width="30" height="30" style="border:0px"></div></div>`);
            halfTargets.push(target);

        } else {
            damageResult.push(`<div class="midi-qol-flex-container"><div class="midi-qol-target-npc midi-qol-target-name" id="${target.id}"> ${target.name} failed with a ${save.total}</div><div><img src="${target.data.img}" width="30" height="30" style="border:0px"></div></div>`);
            fullTargets.push(target);
        }
    }
    new MidiQOL.DamageOnlyWorkflow(actorD, tokenD, damageRoll.total, "force", fullTargets, damageRoll, { flavor: `(Force)`, itemCardId: args[0].itemCardId });
    new MidiQOL.DamageOnlyWorkflow(actorD, tokenD, halfDamage, "force", halfTargets, damageRoll, { flavor: `(Force)`, itemCardId: args[0].itemCardId });
